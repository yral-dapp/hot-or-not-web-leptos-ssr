...
pub async fn get_ml_feed_coldstart_clean(canister_id: Principal, num_results: u32, filter_results: Vec<PostDetails>) -> Result<Vec<PostItem>, anyhow::Error> {
    let client = reqwest::Client::new();
    ...
    if !response.status().is_success() {
        return Err(anyhow::anyhow!(format!("Error fetching ML feed: {:?}", response.text().await?)));
    }
...
pub async fn get_ml_feed_coldstart_nsfw(canister_id: Principal, num_results: u32, filter_results: Vec<PostDetails>) -> Result<Vec<PostItem>, anyhow::Error> {
    let client = reqwest::Client::new();
...
pub async fn get_ml_feed_coldstart_mixed(canister_id: Principal, num_results: u32, filter_results: Vec<PostDetails>) -> Result<Vec<PostItem>, anyhow::Error> {
    let client = reqwest::Client::new();
...
pub async fn get_ml_feed_clean(canister_id: Principal, num_results: u32, filter_results: Vec<PostDetails>) -> Result<Vec<PostItem>, anyhow::Error> {
    let client = reqwest::Client::new();
...
pub async fn get_ml_feed_nsfw(canister_id: Principal, num_results: u32, filter_results: Vec<PostDetails>) -> Result<Vec<PostItem>, anyhow::Error> {
    let client = reqwest::Client::new();
...
pub async fn get_ml_feed_mixed(canister_id: Principal, num_results: u32, filter_results: Vec<PostDetails>) -> Result<Vec<PostItem>, anyhow::Error> {
    let client = reqwest::Client::new();
...
pub fn post_details_to_post_item(post_details: Vec<PostDetails>) -> Vec<PostItem> {
    post_details.into_iter().map(|post_detail| PostItem {
