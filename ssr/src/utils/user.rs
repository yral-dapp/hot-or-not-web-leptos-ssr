...
use leptos::prelude::{use_context, GetUntracked, RwSignal};
...
macro_rules! user_details_or_ret {
    () => {
        if let Some(user) = $crate::utils::user::UserDetails::try_get() {
            user
        } else {
            return;
        }
...
macro_rules! user_details_can_store_or_ret {
    ($e:expr) => {
        if let Some(user) = $crate::utils::user::UserDetails::try_get_from_canister_store($e) {
            user
        } else {
            return;
        }
