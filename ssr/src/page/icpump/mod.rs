...
pub fn TokenListing(details: TokenListItem) -> impl IntoView {
    view! {
        <a href={details.link} class="relative flex h-fit max-h-[300px] w-full gap-2 overflow-hidden border border-transparent p-2 transition-colors hover:border-gray-700 active:border-gray-200">
            ...
            <div class="gap-1 flex-col flex h-fit">
                <div class="flex items-center justify-between gap-4 text-gray-200">
                    <span class="line-clamp-1 w-full overflow-hidden" style="word-break: break-word;">{details.token_name}</span>
                    <span class="shrink-0 font-bold underline"><span class="text-gray-400 italic select-none">"$"</span>{details.token_symbol}</span>
                </div>
                <div title={details.description.clone()} class="text-sm line-clamp-3 text-gray-400" style="word-break: break-word">{details.description}</div>
                <div class="text-xs text-gray-500 line-clamp-2 pr-10">"Created by: "<span class="select-all">{details.user_id}</span><span class="invisible">{details.formatted_created_at.clone()}</span></div>
