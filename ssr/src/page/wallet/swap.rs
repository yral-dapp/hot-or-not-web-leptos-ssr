...
pub fn SwapPage() -> impl IntoView {
  ...
  let wallet_balance = create_signal(100.0);
  ...
  let from_amount = create_signal(0);
  let to_amount = create_signal(0);
  let selected_token = create_signal(None);
  let selected_token_wallet_balance = create_signal(0);
  let selected_token_multiplier: (ReadSignal<f64>, WriteSignal<f64>) = create_signal(0.08);
  
  let is_amount_invalid = create_signal(false);

  let request_swap = create_action(move |_| {
    println!("Request swap");
  });

  let show_select_coin_popup = create_signal(false);
  let show_confirm_popup = create_signal(false);
                    ...
                    <div class="w-full flex flex-col gap-2">
                        <label class="flex bg-[#191919] rounded-lg flex-col px-4 gap-2 py-4 relative hover:bg-zinc-900 focus-within:bg-zinc-900 active:bg-zinc-800">
                            ...
                            <input
                                type="number"
                                on:input=move |e| {
                                    from_amount.set(e.value().parse::<f64>().unwrap_or(0.0));
                                }
                                placeholder="0"
                        ...
                        <label class="flex bg-[#191919] rounded-lg flex-col px-4 gap-2 py-4 relative hover:bg-zinc-900 focus-within:bg-zinc-900 active:bg-zinc-800">
                            ...
                            <input
                                type="number"
                                on:input=move |e| {
                                    to_amount.set(e.value().parse::<f64>().unwrap_or(0.0));
                                }
                            ...
                            <div class="absolute inset-y-0 right-2 z-[5] flex flex-col items-center justify-center">
                                <button
                                    on:click=move || show_select_coin_popup.set(true)
                                    class=format!(
                        ...
                        <div class="flex flex-col gap-1 text-sm font-medium text-[#A0A1A6] pb-5 pt-1">
                            {if (is_amount_invalid.get()) {
                                ...
                                view! {
                                    <div>
                                        "1" {swap_from.name.clone()} "="
                                        {selected_token_multiplier.get()}
                                        {selected_token.get().unwrap().name.clone()}
                                    </div>
                                }
                            }}
                            <div>
                                "1" {swap_from.name.clone()} "="
                                {(to_amount.get() / selected_token_multiplier.get()).to_string()}
                                {selected_token.get().unwrap().name.clone()}
                            </div>
                        </div>

                        <Button
                            on:click=move || request_swap.dispatch(())
                            disabled=!selected_token.get().is_some() || !from_amount.get()
                        >
                            0.0
                            >
                            "Request swap"
...
pub fn SelectCoinPopup() -> impl IntoView {
  ...
  let coin_search_query = create_signal("".to_string());

  let list_of_coins = create_signal(vec![]);
                ...
                <div class="bg-[#131313] flex flex-col py-4 text-white h-full w-full overflow-y-auto shadow-md rounded-lg">
                    <div class="px-4 pb-4 border-b border-zinc-900">
                        <input
                            ...
                            on:input=move |e| {
                                coin_search_query.set(e.value().to_string());
                            }
                            type="text"
                    ...
                    {if (list_of_coins.get().len() > 0) {
                        ...
                        view! { <div class="text-zinc-500 text-center py-4">No results found</div> }
                    }}
                </div>
            </div>
        </div>
...
fn ConfirmPopup() -> impl IntoView {
  ...
  let from_token = create_signal(None);
  let to_token = create_signal(None);
      ...
      <div class="fixed z-[999] inset-0">
          ...
          >
              <div class="bg-[#191919] relative flex flex-col gap-1 text-white items-center shadow-md rounded-sm p-8">
                  <button
                      on:click=move || show_confirm_popup.set(false)
                  ...
                  <div class="flex items-center gap-4 pt-8">
                      <img
                          src=from_token.get().unwrap().icon
                          alt=from_token.get().unwrap().name
                          class="w-20 h-20 rounded-full"
                      />
                      <LongArrowRightIcon classes="w-8 text-zinc-500".to_string() />
                      <img
                          src=to_token.get().unwrap().icon
                          alt=to_token.get().unwrap().name
                          class="w-20 h-20 rounded-full"
                      />
                  </div>
                  <div class="text-xl font-bold text-center pt-3">
                      {from_token.get().unwrap().name.clone()}to
                      {to_token.get().unwrap().name.clone()}
                  </div>
