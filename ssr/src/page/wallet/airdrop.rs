...
pub fn AirdropPage() -> impl IntoView {
  let bg_img_loaded = create_signal(false);
  let claimed = create_signal(false);
  let coin_image_loaded = create_signal(false);
  ...
  let handle_claim = move |_| {
    ...
    >
      <img
        ...
        src=bg_image
        on:load=on_img_load
        class="absolute inset-0 z-[1] w-full h-full object-cover transition-all duration-1000 {bg_img_loaded
      ...
      {move || {
            ...
            <div style="--y: 50px" class="flex flex-col items-center justify-center airdrop-parachute">
              ...
              >
                <img
                  alt="Airdrop"
                  src=coin_image
                  on_load={() => (coin_image_loaded.set(true))}
                  class=format!("w-full rounded-full h-full object-cover transition-opacity {}", if coin_image_loaded.get() { "opacity-100" } else { "opacity-0" })
                />
              </div>
            </div>
        ...
        return view! {
          ...
          <div class="h-[12rem] w-[12rem] relative">
            <AnimatedTick />
            <div
              style="background: radial-gradient(circle, rgba(27,0,15,1) 0%, rgba(0,0,0,1) 100%); box-shadow: 0px 0px 3.43px 0px #FFFFFF29;"
              class="p-[1px] absolute w-16 h-16 -bottom-4 -right-4 rounded-full"
            >
              <img
                alt="Airdrop"
                src=coin_image
      ...
      {move || {
        if bg_img_loaded.get() {
          return view! {
            <div
            in:fly={{ delay: 1200, duration: 1500, y: 50 }}
            class="flex text-xl font-bold z-[2] w-full flex-col gap-4 items-center justify-center px-8"
          >
            {#if claimed}
              <div class="text-center">
                "100 Rock_Salt" <br /><span class="font-normal">"added to wallet"</span>
              </div>
            {:else}
              <div class="text-center">"100 Rock_Salt Airdrop received received received"</div>
            {/if}
      
            
              <Button slot="label" class="max-w-96 mx-auto" altStyle=claimed.get() on_click=handle_claim>
