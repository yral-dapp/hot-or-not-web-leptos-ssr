...
impl<'a> Service<'a> {
  pub async fn get_top_posts_aggregated_from_canisters_on_this_network_for_home_feed(
    ...
    let bytes = self.1.query(&self.0, "get_top_posts_aggregated_from_canisters_on_this_network_for_home_feed").with_arg(args).call().await?;
  ...
  pub async fn get_top_posts_aggregated_from_canisters_on_this_network_for_hot_or_not_feed(
    ...
    let args = Encode!(&arg0, &arg1)?;
    let bytes = self.1.query(&self.0, "get_top_posts_aggregated_from_canisters_on_this_network_for_hot_or_not_feed").with_arg(args).call().await?;
    Ok(Decode!(&bytes, Result_)?)
  }
